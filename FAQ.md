...
We don't release new versions of Tesser on F-Droid ourselves. Once we've released
...
### Can you port Tesser to iOS/Windows/MacOS/Browser Extension?

We don't have plans to port Tesser to other platforms.
...
adding support for this feature in Tesser.
...
If you could previously unlock Tesser with biometrics, but suddenly can't do so
...
settings of Tesser and re-enable it.

### Why does Tesser keep prompting me for my password, even though I have enabled biometric authentication?

You're probably encountering the password reminder. Try entering your password
to unlock the vault once. After that, Tesser will prompt for biometrics by
...
the vault, **we do NOT recommend disabling the password reminder**.

### Tesser uses SHA1 for most/all of my tokens. Isn't that insecure?
...
were to change this on Tesser' end, the tokens would stop working. Furthermore,
when using SHA1 in an HMAC calculation, the currently known issues in SHA1 are
not of concern.

### Why doesn't Tesser support biometric unlock for my device, even though it works with other apps?

The reason for this is pretty technical. In short, since you're not entering
your password when using biometric unlock, Tesser needs some other way to decrypt
...
resulting in the error displayed to you by Tesser in an error dialog.

If biometrics works with other apps, but not with Tesser, that means those other
apps probably perform a weaker form of biometric authentication.

## Backups

### How can I back up my Tesser vault to the cloud automatically?

Tesser can only automatically back up to the cloud if the app of your cloud
provider is installed on your device and fully participates in the Android
Storage Access Framework. Tesser doesn't have access to the internet and we don't
...
Another common setup is to configure Tesser to back up to a folder on local
...
Tesser uses the same password you have configured to encrypt your vault as the
...
**Settings -> Backup & Restore -> Export as Text and HTML**. The `.db` format
is not supported.
...
without a password and then importing that into Tesser. Another option is
extracting the ZIP archive on a computer and importing the resulting
Accounts.txt file into Tesser with the "Plain text" import option.
